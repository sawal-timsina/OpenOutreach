...
- **`linkedin_cli/`**: Django-free library of LinkedIn platform mechanics (browser nav/login, Voyager API, profile/conversation scrape, connect/message/status/thread verbs), imports with no Django. Exposes a JSON verb CLI (`python -m linkedin_cli.cli`: `session open/close`, `login`, `whoami`, `profile`, `status`, `connect`, `message`, `thread`) over a **bind+connect** transport — a session owner `browser.bind()`s the browser, clients `chromium.connect()`. The daemon owns its browser in-process and consumes the library directly; launch/cookie-persistence glue stays in `linkedin/browser/launch.py`. CRM/campaign/scheduling/ML stay in this project. (Roadmap: `extract-linkedin-cli`.)
